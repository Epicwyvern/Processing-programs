...
void mouseReleased () {
  ...
  if (bookmarkState==3 && mouseX > width/1.627118644 && mouseX < width/1.627118644+100 && mouseY > height/3+20 && mouseY < height/3+20+100) {
    ...
    int x = 0;
    int max = bookmarks.length-1;
    int min = 0;
    int mid = (max + min)/2;  
    int [] ints =  new int[bookmarks.length];
    ...
    cp5s3.get(ScrollableList.class, "Title_Author_Page").clear();
    cp5s3.get(ScrollableList.class, "Title_Author_Page").addItems(str(ints));
      
    x = int(cp5s3.get(Textfield.class, "").getText());
    
    
  while( min <= max){  
      if ( ints[mid] < x ){  
        min = mid + 1;     
      }else if (ints[mid] == x && x != 0 ){  
        System.out.println("Element is found at index: " + mid);  
        cp5s3.get(ScrollableList.class, "Title_Author_Page").clear();
        cp5s3.get(ScrollableList.class, "Title_Author_Page").addItem(str(ints[mid]),1);
        break;  
      }else{  
         max = mid - 1;  
      }  
